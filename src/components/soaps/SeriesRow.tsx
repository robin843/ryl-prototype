...
export function SeriesRow({ title, series }: SeriesRowProps) {
  ...
  return (
    <section className="mb-8">
      {/* Category Title */}
      <h2 className="text-headline text-base sm:text-lg font-medium px-5 sm:px-12 mb-3">{title}</h2>
      ...
      <div className="relative group">
        ...
        <div
          ...
          className="flex gap-3 sm:gap-4 overflow-x-auto scrollbar-hide scroll-smooth snap-x"
        >
          {series.map((s, index) => (
            <Link
              key={s.id}
