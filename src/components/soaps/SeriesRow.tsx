...
export function SeriesRow({ title, series }: SeriesRowProps) {
  ...
  return (
    <section className="mb-8">
      {/* Category Title */}
      <h2 className="text-headline text-base sm:text-lg font-medium px-6 sm:px-10 mb-3">{title}</h2>
      ...
      <div className="relative group">
        ...
        <div
          ref={scrollRef}
          className="flex gap-3 sm:gap-4 overflow-x-auto scrollbar-hide pl-6 sm:pl-10 pr-5 scroll-smooth snap-x snap-mandatory"
        >
          {series.map((s, index) => (
            <Link
              key={s.id}
